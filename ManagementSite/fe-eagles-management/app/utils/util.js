 ...
 */
  getFormatTime(timestamp) {
    ...
    const minutes = Math.floor(parseInt(today - new Date(timestamp)) / 1000 / 60);// 距离现在的间隔的分钟数
 ...
 */
  setLoaclStorage(token, refresh_token) {
    let old_localStorage = localStorage.info ? JSON.parse(localStorage.info) : {};
    let info = {
      ...old_localStorage,
      token,
      refresh_token
  ...
  lightKeyword(content, keyword) {
    ...
    if (content && keyword) {
      ...
      return content.replace(pattern, `<font style='color:#fe0000'>${keyword}</font>`);
    } else {
      return content;
    }
  },
